  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
        ...
        console.log('HLS Levels parsed:', levels);
        setQualities(levels);
      ...
      hls.on(Hls.Events.LEVEL_SWITCHED, (_, data) => {
        // Only update currentQuality for internal tracking, never touch displayQuality
                ...
                <div className="absolute bottom-full right-0 mb-2 bg-gray-900/95 rounded-lg overflow-hidden min-w-[120px] shadow-lg">
                  ...
                  {qualities.length > 0 ? (
                    [...qualities]
                      ...
                      .map((q) => (
                        <button
                          key={`quality-${q.height}-${q.index}`}
                          onClick={() => {
                            console.log('Clicked quality:', q.height, 'HLS index:', q.index);
                            changeQuality(q.index, q.height);
                          }}
                          className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-700 flex items-center justify-between ${
